...
import { adherenceInputSchema, type AdherenceInput } from "@/contracts/adherence-api";
...
type Payload = { jobId: string; organizationId: string; input: AdherenceInput };

export async function queueAdherence(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.ADHERENCE_DETECTION_ENABLED?.trim() === "false") {
    return apiError(
      "adherence_disabled",
      "Adherence detection API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = adherenceInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const { data: job, error: jobInsertError } = await supabase
    ...
    .insert({
      ...
      type: "adherence_detection",
      status: "queued",
      payload: {
        phase: "queued",
        input: {
          has_notes: Boolean(parsed.data.input.notes),
  ...
  if (jobInsertError || !job) {
    return apiError("job_create_failed", "Failed to create adherence job.", 500, request.requestId, "api_error");
  }

  try {
    await tasks.trigger("adherence-analyzer", {
      jobId: job.id,
