...
import { patientRiskInputSchema, type PatientRiskInput } from "@/contracts/patient-risk-api";
...
type Payload = { jobId: string; organizationId: string; input: PatientRiskInput };

export async function queuePatientRisk(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.PATIENT_RISK_PRIORITY_ENABLED?.trim() === "false") {
    return apiError(
      "patient_risk_disabled",
      "Patient risk & priority API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = patientRiskInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const scope = "health_risk_priority";
  ...
  const { data: job, error: jobInsertError } = await supabase
    ...
    .insert({
      ...
      type: "patient_risk_priority",
      status: "queued",
      payload: {
        phase: "queued",
        input: {
          has_image: Boolean(parsed.data.input.image_data),
  ...
  if (jobInsertError || !job) {
    return apiError("job_create_failed", "Failed to create patient risk job.", 500, request.requestId, "api_error");
  }
  ...
  try {
    await tasks.trigger("patient-risk-analyzer", triggerPayload);
  } catch (error) {
    await supabase
      .from("jobs")
      .update({
