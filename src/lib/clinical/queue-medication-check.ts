...
export async function queueMedicationCheck(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.MEDICATION_CHECK_ENABLED?.trim() === "false") {
    return apiError(
      "medication_check_disabled",
      "Medication intelligence API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = medicationCheckInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const scope = "health_medication_check";
  ...
  const p: { jobId: string; organizationId: string; input: MedicationCheckInput } = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("medication-check-analyzer", p);
  } catch (e) {
    await supabase
      .from("jobs")
      .update({
