...
type CognitiveWellnessAnalyzerPayload = {
  jobId: string;
  organizationId: string;
  input: CognitiveWellnessInput;
};

export async function queueCognitiveWellness(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.COGNITIVE_WELLNESS_ENABLED?.trim() === "false") {
    return apiError(
      "cognitive_wellness_disabled",
      "Cognitive wellness API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = cognitiveWellnessInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const { data: job, error: jobInsertError } = await supabase
    ...
    .insert({
      ...
      type: "cognitive_wellness_analysis",
      status: "queued",
      payload: {
        phase: "queued",
        input: {
  ...
  if (jobInsertError || !job) {
    return apiError(
      "job_create_failed",
      "Failed to create cognitive wellness job.",
      500,
      request.requestId,
      "api_error"
    );
  ...
  const triggerPayload: CognitiveWellnessAnalyzerPayload = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("cognitive-wellness-analyzer", triggerPayload);
  } catch (error) {
    await supabase
      .from("jobs")
      .update({
