...
import { scanAnalysisInputSchema, type ScanAnalysisInput } from "@/contracts/scan-analysis-api";
...
export async function queueScanAnalysis(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.SCAN_ANALYSIS_ENABLED?.trim() === "false") {
    return apiError(
      "scan_analysis_disabled",
      "Scan analysis API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = scanAnalysisInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const scope = "health_scan_analysis";
  ...
  const { data: job, error: insErr } = await supabase
    ...
    .insert({
      ...
      payload: {
        phase: "queued",
        input: { modality: parsed.data.input.modality },
      },
    })
    .select("id")
    .single();
  ...
  const p: { jobId: string; organizationId: string; input: ScanAnalysisInput } = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("scan-analysis-analyzer", p);
  } catch (e) {
    await supabase
      .from("jobs")
      .update({
