...
import { researchAssistInputSchema, type ResearchAssistInput } from "@/contracts/research-assist-api";
...
export async function queueResearchAssist(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.RESEARCH_ASSIST_ENABLED?.trim() === "false") {
    return apiError(
      "research_assist_disabled",
      "Research assist API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = researchAssistInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const scope = "health_research_assist";
  const h = sha(stableStringify(parsed.data));
  ...
  const { data: job, error: insErr } = await supabase
    ...
    .insert({
      ...
      payload: {
        phase: "queued",
        input: { has_dataset: Boolean(parsed.data.input.dataset) },
      },
    })
    .select("id")
    .single();
  ...
  if (insErr || !job) {
    return apiError("job_create_failed", "Failed.", 500, request.requestId, "api_error");
  }

  const p: { jobId: string; organizationId: string; input: ResearchAssistInput } = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("research-assist-analyzer", p);
  } catch (e) {
    await supabase
      .from("jobs")
      .update({
