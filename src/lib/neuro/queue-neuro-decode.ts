...
import { neuroDecodeInputSchema, type NeuroDecodeInput } from "@/contracts/neuro-decode-api";
...
type NeuroDecodeAnalyzerPayload = {
  jobId: string;
  organizationId: string;
  input: NeuroDecodeInput;
};

export async function queueNeuroDecode(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.NEURO_DECODE_ENABLED?.trim() === "false") {
    return apiError(
      "neuro_decode_disabled",
      "Neural decode API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = neuroDecodeInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const { data: job, error: jobInsertError } = await supabase
    ...
    .insert({
      ...
      payload: {
        phase: "queued",
        input: {
          paradigm: parsed.data.input.paradigm,
  ...
  if (jobInsertError || !job) {
    return apiError("job_create_failed", "Failed to create neuro decode job.", 500, request.requestId, "api_error");
  }

  const triggerPayload: NeuroDecodeAnalyzerPayload = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("neuro-decode-analyzer", triggerPayload);
  } catch (error) {
    await supabase
      .from("jobs")
      .update({
