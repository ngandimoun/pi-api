...
import { streamCreateInputSchema, type StreamCreateInput } from "../../contracts/surveillance-api";
import { apiError, apiZodError } from "../api-response";
import { mergeStreamConfig } from "./profiles";
...
export type SurveillanceStreamAnalyzerPayload = {
  jobId: string;
  organizationId: string;
  input: StreamCreateInput;
};

export async function queueStreamAnalysis(request: AuthenticatedRequest): Promise<Response> {
  ...
  const parsed = streamCreateInputSchema.safeParse(body);
  if (!parsed.success) {
    return apiZodError("invalid_request_body", parsed.error, 400, request.requestId);
  }

  const merged = mergeStreamConfig(parsed.data, parsed.data.profile);

  const supabase = getServiceSupabaseClient();
  const idempotencyKey = request.headers.get("idempotency-key")?.trim() ?? "";
  const scope = "surveillance_streams";
  ...
  const { data: job, error: jobInsertError } = await supabase
    ...
    .insert({
      ...
      type: "surveillance_stream_analysis",
      status: "queued",
      payload: {
        phase: "queued",
        input: {
          has_frame: Boolean(merged.input?.data),
  ...
  if (jobInsertError || !job) {
    return apiError("job_create_failed", "Failed to create surveillance job.", 500, request.requestId, "api_error");
  }

  const triggerPayload: SurveillanceStreamAnalyzerPayload = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("surveillance-stream-analyzer", triggerPayload);
  } catch (error) {
    await supabase
      .from("jobs")
      .update({
