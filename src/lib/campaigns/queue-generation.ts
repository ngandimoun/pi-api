...
import { campaignAdGenerationInputSchema, type CampaignAdGenerationInput } from "@/contracts/campaign-ads-api";
...
type CampaignAdsCreatorPayload = {
  jobId: string;
  organizationId: string;
  input: CampaignAdGenerationInput;
};

export async function queueCampaignGeneration(request: AuthenticatedRequest): Promise<Response> {
  let body: unknown;
  try {
  ...
  const parsed = campaignAdGenerationInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const scope = "campaigns_generate";
  ...
  const triggerPayload: CampaignAdsCreatorPayload = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("campaign-ads-creator", triggerPayload);
  } catch (error) {
    await supabase
      .from("jobs")
      .update({
