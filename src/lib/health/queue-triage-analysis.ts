...
import { healthTriageInputSchema, type HealthTriageInput } from "@/contracts/health-triage-api";
...
type HealthTriageAnalyzerPayload = {
  jobId: string;
  organizationId: string;
  input: HealthTriageInput;
};

export async function queueHealthTriageAnalysis(request: AuthenticatedRequest): Promise<Response> {
  if (process.env.HEALTH_TRIAGE_ENABLED?.trim() === "false") {
    return apiError(
      "health_triage_disabled",
      "Health triage API is disabled.",
      403,
      request.requestId,
      "permission_error"
    );
  ...
  const parsed = healthTriageInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const { data: job, error: jobInsertError } = await supabase
    ...
    .insert({
      ...
      type: "health_triage_analysis",
      status: "queued",
      payload: {
        phase: "queued",
        input: {
          type: parsed.data.input.type,
          modality: parsed.data.input.modality ?? null,
  ...
  if (jobInsertError || !job) {
    return apiError("job_create_failed", "Failed to create triage job.", 500, request.requestId, "api_error");
  }

  const triggerPayload: HealthTriageAnalyzerPayload = {
    jobId: job.id,
  ...
  try {
    await tasks.trigger("health-triage-analyzer", triggerPayload);
  } catch (error) {
    await supabase
      .from("jobs")
      .update({
