...
import { step5LocalizeOutputSchema } from "@/mastra/workflows/campaign-localize/schemas";
...
function deterministicCompiledTextPrompt(campaignPromptGenerator: Record<string, unknown>, fallbackPrompt: string): string {
...
export const step5LocalizedJsonPrompt = createStep({
  id: "campaign-localize-step5-localized-json-prompt",
  inputSchema: z.any(),
  outputSchema: z.any(),
  execute: async ({ inputData }) => {
    ...
    try {
        ...
        contents: [
          {
            text: [
              ...
              `TARGET_CULTURE: ${inputData.step2.localization_brief.target_culture}`,
              `TARGET_LANGUAGE: ${inputData.step2.localization_brief.target_language}`,
              `TARGET_CURRENCY: ${inputData.step2.localization_brief.target_currency}`,
              `LAYOUT_PRESERVATION: ${inputData.step2.localization_brief.layout_preservation_instructions}`,
              `SOURCE_IMAGE_ANALYSIS: ${JSON.stringify(inputData.step1.source_image_analysis)}`,
              `CULTURAL_RETRIEVAL_REFERENCE_URL: ${inputData.step3.corpus_image_url}`,
              `CULTURAL_REASONING: ${JSON.stringify(inputData.step4)}`,
              `BRAND_DNA: ${JSON.stringify(inputData.step1.brand_context?.brand_dna ?? null)}`,
              "Fill placeholders with culturally resonant values, but do not violate structural layout constraints.",
            ].join("\n"),
          },
        ],
      });
      ...
      const compiledBase = deterministicCompiledTextPrompt(normalized.campaign_prompt_generator, inputData.input.prompt);
      const compiledTextPrompt = [
        "LOCALIZATION_CONSTRAINTS:",
        `- Preserve exact structural layout from source image. ${String(inputData.step4.structural_layout_map)}`,
        `- Apply cultural replacements: ${(inputData.step4.cultural_replacements ?? []).join(" | ")}`,
        `- Text direction: ${String(inputData.step4.text_direction ?? "LTR")}`,
        `- Currency: ${String(inputData.step2.localization_brief.target_currency)}. ${String(inputData.step4.currency_format_notes ?? "")}`,
        "",
        compiledBase,
      ].join("\n");

      const step5 = step5LocalizeOutputSchema.parse({
        json_prompt: normalized.campaign_prompt_generator,
        compiled_text_prompt: compiledTextPrompt,
      });

      const diagnostic = {
        step: "campaign-localize-step5-localized-json-prompt",
        status: "ok" as const,
        duration_ms: Date.now() - startedAt,
        detail: {
          compiled_prompt_chars: step5.compiled_text_prompt.length,
        },
      };
      ...
      await appendJobDiagnostic({
        ...
        diagnostic,
        phase: "campaign-localize-step5-localized-json-prompt",
      });

      return { ...inputData, step5, diagnostics };
    } catch (error) {
      const diagnostic = {
        step: "campaign-localize-step5-localized-json-prompt",
