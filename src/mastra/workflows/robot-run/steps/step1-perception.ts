...
import type { PerceptionResult } from "../../../../contracts/surveillance-api";
...
export const step1Perception = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    const streamLike = {
      ...
      behaviors: [],
      anomaly: undefined,
      outputs: { delivery: ["sse"], format: "summary" as const },
      alerts: { min_severity: "info" as const, cooldown_seconds: 30, group_by: "zone" as const },
      profile: undefined,
      context: input.context,
      input: input.perception?.input,
      frame_index: input.perception?.frame_index ?? 0,
      output: { include_diagnostics: false, format: "json" as const },
    };
