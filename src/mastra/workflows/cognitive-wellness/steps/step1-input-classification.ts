import { createStep } from "@mastra/core/workflows";
import { z } from "zod";

import { finishDiagnostic, startTimer } from "@/mastra/workflows/health-triage/debug";
import {
  cognitiveWellnessWorkflowInputSchema,
  step1WellnessOutputSchema,
} from "@/mastra/workflows/cognitive-wellness/schemas";

const DEFAULT_PARADIGM = "cognitive_wellness";

function resolveLocale(input: {
  output?: { locale?: string } | undefined;
  context?: Record<string, unknown> | undefined;
}): string | undefined {
  const fromOutput = input.output?.locale?.trim();
  if (fromOutput) return fromOutput;
  const ctx = input.context ?? {};
  const raw = (ctx["locale"] ?? ctx["language"] ?? ctx["lang"]) as unknown;
  return typeof raw === "string" && raw.trim().length > 0 ? raw.trim().slice(0, 32) : undefined;
}

export const step1WellnessInputClassification = createStep({
  id: "cognitive-wellness-step1-input-classification",
  inputSchema: cognitiveWellnessWorkflowInputSchema,
  outputSchema: z.any(),
  execute: async ({ inputData }) => {
    const started = startTimer();
    const inp = inputData.input.input;
    const modality = inp.modality?.trim();
    const paradigm = modality && modality.length > 0 ? modality : DEFAULT_PARADIGM;
    const locale = resolveLocale({
      output: inputData.input.output,
      context: inputData.input.context,
    });
    const has_image = Boolean(inp.image_data?.trim());

    const step1 = step1WellnessOutputSchema.parse({
      paradigm,
      locale,
      has_image,
      device: inp.device?.trim(),
      channels: inp.channels,
      sample_rate: inp.sample_rate,
    });

    return {
      ...inputData,
      step1,
      diagnostics: [
        ...(inputData as { diagnostics?: unknown[] }).diagnostics ?? [],
        finishDiagnostic({
          step: "step1_input_classification",
          started_at: started,
          status: "ok",
          detail: { paradigm, has_image },
        }),
      ],
    } as Record<string, unknown>;
  },
});
