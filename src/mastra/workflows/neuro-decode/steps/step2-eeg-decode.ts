...
import { step2NeuroOutputSchema } from "@/mastra/workflows/neuro-decode/schemas";

export const step2NeuroEegDecode = createStep({
  id: "neuro-decode-step2-eeg-decode",
  inputSchema: z.any(),
  outputSchema: z.any(),
  execute: async ({ inputData }) => {
    ...
    try {
      const { result, routing_fallbacks } = await decodeEeg({
        requestId: jobId,
      ...
      const step2 = step2NeuroOutputSchema.parse({
