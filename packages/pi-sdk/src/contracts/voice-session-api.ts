...
import { voiceAgentVoiceSchema } from './voice-agent-api.js';
...
export const voiceSessionCreateInputSchema = z
  ...
  .superRefine((data, ctx) => {
    if (
      data.max_duration_seconds != null &&
      ...
      ctx.addIssue({
        ...
        message:
          'ttl_seconds must be greater than or equal to max_duration_seconds when both are provided.',
        path: ['ttl_seconds'],
      });
    }
  });
